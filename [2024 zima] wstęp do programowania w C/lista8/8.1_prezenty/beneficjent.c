...
{
    if (osoba == NULL) return;
    printf("Osoba nazywa sie %s, ma %hhu lat, %u zaslug i %u szelmostw\n", osoba->imie, osoba->wiek, osoba->zaslugi, osoba->szelmostwa);
}
int komparator(const struct Osoba* os1, const struct Osoba* os2)
{
    unsigned int roznica1 = os1->zaslugi - os1->szelmostwa;
    unsigned int roznica2 = os2->zaslugi - os2->szelmostwa;
