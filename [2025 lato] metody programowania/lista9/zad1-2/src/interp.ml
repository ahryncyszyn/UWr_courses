...
let rec eval_env (env : env) (e : expr) (h : heap) : value * heap =
  ...
  (* | While (cond, body) ->                     (* zad 1 - petla while *)
    let rec while_loop h =
      let (evaluated_cond, h') = eval_env env cond h in
      match evaluated_cond with
      | VBool true -> let (_, h'') = eval_env env body h' in while_loop h''
      | VBool false -> (VUnit, h')
      | _ -> failwith "type error in condition"
    in while_loop h *)
  | While (cond, body) ->                      (* zad 2 - petla while *)
    let rec while_loop h =
      let (evaluated_cond, h') = eval_env env cond h in
      match evaluated_cond with
      | VBool true -> 
          (try let (_, h'') = eval_env env body h' in while_loop h'' 
            with
           | Break -> (VUnit, h')
           | Continue -> while_loop h')
      | VBool false -> (VUnit, h')
      | _ -> failwith "type error in condition"
    in while_loop h
  | Break -> raise Break
