...
let from_rpn (prog : prog) : Ast.expr =
  let rec aux (stack : Ast.expr list) (prog : cmd list) : Ast.expr =
    match prog, stack with
    | PushInt n :: rest, _ -> aux (Ast.Int n :: stack) rest
    | Binop op :: rest, e2 :: e1 :: stack' -> aux (Ast.Binop (op, e1, e2) :: stack') rest
    | [], [e] -> e
    | _ -> failwith "invalid RPN program"
  in aux [] prog

let test = from_rpn [PushInt 1; PushInt 2; Binop Add];;