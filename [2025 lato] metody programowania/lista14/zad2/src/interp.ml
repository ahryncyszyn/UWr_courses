...
type heap = value H.t
type heaps = heap list            (* zad 2 *)

(* type 'a comp = heap -> a' * heap *)
type 'a comp = heaps -> 'a * heaps  (* zad 2 *)
...
let enter_scope : unit comp = 
  fun hs: heaps -> ((), H.empty :: hs)

let leave_scope : unit comp =
  fun hs: heaps -> ((), List.tl hs)

let deref (l : ident) : value comp =                    (* zad 2 *)
  fun h -> let rec help (hs : heaps) : value =
    match hs with
    | [] -> failwith("brak zmiennej")
    | x::xs -> 
      (match (H.find_opt l x) with
      | Some x -> x
      | None -> help xs)
    in (help h, h)

let assgn (l : ident) (v : value) : unit comp =
  fun h -> let rec help (hs : heaps) : heaps =
    match hs with
    | [] -> failwith("brak zmiennej")
    | [x] -> [H.add l v x]
    | x::xs -> 
      (match (H.find_opt l x) with
      | Some xs -> (H.add l v x)::xs
      | None -> x::help xs)
    in ((), help h)

let local (l : ident) (v : value) : unit comp =
  fun h -> let rec help (hs : heaps) : heaps =
    match hs with
    | [] -> failwith("brak zmiennej")
    | x::xs -> (H.add l v x)::xs
