...
params:                                                     (* zad 3 *)
    | LPAREN; x = IDENT; COLON; tp = type_expr; RPAREN
        { [ (x, tp) ] }
    | LPAREN; x = IDENT; COLON; tp = type_expr; RPAREN; ps = params
        { (x, tp) :: ps }
...
mexpr:
    ...
    | FUNREC; f = IDENT; ps = params; COLON; t_ret = type_simpl; ARR; body = mexpr      (* zad 3 *)
    {
        match ps with
        | (x1, t1) :: rest ->
            let curried_body = List.fold_right (fun (x, t) acc -> node $loc (Fun(x, t, acc))) rest body
            in let curried_type = List.fold_right (fun (_, t) acc -> TArrow(t, acc)) rest t_ret
            in node $loc (Funrec(f, x1, t1, curried_type, curried_body))
...
(*
let src = " funrec fact (n : int) (r : int) : int -> if n = 0 then r else fact (n - 1) (r * n)";;
