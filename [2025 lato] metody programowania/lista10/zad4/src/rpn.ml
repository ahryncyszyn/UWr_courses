...
let rec of_ast (env : env) (expr : Ast.expr) : prog =
  ...
  | Ast.Var x -> [PushVar x]  (* zad 4  *)
  | Ast.Let (x, e1, e2) -> of_ast env e1 @ [Store] @ of_ast (x :: env) e2 @ [CleanUp]
 ...
 *)
let rec eval (env : I.env) (s : I.value list) (p : prog) : I.value =
  match p, s with
  | [], [n] -> n
  | [], _   -> failwith "error!"
  | (PushVar x :: p'), _ ->                           (* zad 4 *)
    (match I.M.find_opt x env with
    | Some v -> eval env (v :: s) p'
    | None -> failwith ("nieznana zmienna")
    | _ -> failwith ("blad"))
  | (PushInt n :: p'), _ ->
